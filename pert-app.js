...
(() => {
  const EXAMPLE = {
    ...
    tasks: [
      ...
      { id: "H", name: "Lancement", duration: 2, predecessors: ["F"] },
    ],
  };
  ...
  els.projectTitle.addEventListener("blur", () => {
    ...
    if (!txt) {
      els.projectTitle.textContent = currentData.title || "Diagramme PERT";
      return;
    }
  ...
  els.resetBtn.addEventListener("click", () => {
    if (
      !confirm("Charger l'exemple ? Les données actuelles seront remplacées.")
    )
      return;
  ...
  function buildTaskCard(task, idx) {
    ...
    card
      ...
      .forEach((i) => i.addEventListener("input", onFieldInput));
    return card;
  }
  ...
  function onFieldInput(e) {
    ...
    if (field === "predecessors") {
      task.predecessors = e.target.value
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
  ...
  els.viewport.addEventListener("mousedown", (e) => {
    if (e.target.closest("#zoom-controls") || e.target.closest("#node-legend"))
      return;
    isDragging = true;
    lastX = e.clientX;
    lastY = e.clientY;
  ...
  if (dirHintEl) {
    requestAnimationFrame(() => {
      ...
      dirHintEl.style.bottom =
        vpR.bottom -
        btnR.bottom +
        (btnR.height - dirHintEl.offsetHeight) / 2 +
        "px";
    });
    setTimeout(() => {
